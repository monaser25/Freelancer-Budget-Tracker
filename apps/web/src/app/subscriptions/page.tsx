...
export default function SubscriptionsPage() {
  ...
  const visibleSubscriptions = useMemo(
    () => subscriptions.filter((sub) => !sub.archivedAt),
    [subscriptions],
  );
  const totalMonthlyCost = useMemo(() => visibleSubscriptions.filter((sub) => sub.status === 'ACTIVE').reduce((sum, sub) => sum + monthlyEquivalent(sub), 0), [visibleSubscriptions]);
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[220] bg-slate-900/40 flex items-start sm:items-center justify-center overflow-y-auto p-4" onMouseDown={closeDeleteModal}>
          <div className="bg-white rounded-[var(--radius-xl)] border border-border shadow-xl w-full max-w-[460px] max-h-[calc(100vh-2rem)] overflow-y-auto p-5 sm:p-6" onMouseDown={(event) => event.stopPropagation()}>
            <h2 className="text-[16px] font-semibold text-textPrimary">Remove {deleteTarget.subscription.name}?</h2>
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 pt-5 mt-5 border-t border-border">
              ...
              <button type="button" disabled={isDeleting} onClick={confirmDelete} className="px-4 py-2 rounded-md bg-red-600 text-white text-[13px] font-medium hover:bg-red-700 disabled:opacity-60">{isDeleting ? 'Removing...' : 'Remove Subscription'}</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); onSave(new FormData(event.currentTarget), subscription); }} className="space-y-4">
      <div>
        ...
        {subscription && <p className="text-[13px] text-amber-600 mt-1">Changes to amount, cycle, or billing date only affect future billings. Past transactions remain unchanged.</p>}
