...
const fmtDate = (v: Date | string) => {
  const d = new Date(v);
  return Number.isNaN(d.getTime()) ? '—' : formatDate(d, DEFAULT_LOCALE, { month: 'short', day: 'numeric', year: 'numeric' });
};
...
const makeMoney = (currency: string) => {
  ...
  return (n: number) => {
    const amount = formatNumber(Number.isFinite(n) ? n : 0, DEFAULT_LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
export async function buildInvoicePdf(data: InvoicePdfData): Promise<Buffer> {
  ...
  right(`Generated ${formatDate(new Date(), DEFAULT_LOCALE)}`, RIGHT, 40, 9, font, MUTED);
